...
from ..modules.face_model import FaceModel
from ..modules.id_encoder import FusionFaceId
from ..models.wan_video_animate_adapter import WanAnimateArcfaceAdapter
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.in_iteration_models = ("dit", "motion_controller", "vace", "animate_adapter", "vap", "face_encoder", "animate_arcface_adapter")
        self.in_iteration_models_2 = ("dit2", "motion_controller", "vace2", "animate_adapter", "vap", "face_encoder", "animate_arcface_adapter")
        self.units = [
            WanVideoUnit_ShapeChecker(),
            WanVideoUnit_NoiseInitializer(),
            WanVideoUnit_PromptEmbedder(),
            WanVideoUnit_InputVideoEmbedder(),
            WanVideoUnit_ImageEmbedderVAE(),    # input_image
            WanVideoUnit_ImageEmbedderCLIP(),   # input_image
        ...
        self.face_encoder: FusionFaceId = None
        self.animate_arcface_adapter: WanAnimateArcfaceAdapter = None
        self.face_units = [
            WanVideoUnit_ArcfaceEmbedder(),
        ]
    ...
    def from_pretrained(
        ...
        # New Feature: Arcface Adapter
        if face_model_path is not None:
            # TODO: move to GPU
            pipe.face_model = FaceModel(face_model_path=face_model_path, device="cpu")
        else:
            pipe.face_model = None
        pipe.face_encoder = FusionFaceId().to(device=device, dtype=torch_dtype)
        pipe.animate_arcface_adapter = WanAnimateArcfaceAdapter().to(device=device, dtype=torch_dtype)
        
        # Size division factor
        if pipe.vae is not None:
            pipe.height_division_factor = pipe.vae.upsampling_factor * 2    # 16
            pipe.width_division_factor = pipe.vae.upsampling_factor * 2     # 16
    ...
    def __call__(
        ...
        # TODO: Animate Arcface
        animate_ref_identity_image: Optional[Image.Image] = None,
    ):
        # Scheduler
        ...
        inputs_shared = {
            ...
            "vap_video": vap_video, 
            # TODO: Animate Arcface
            "animate_ref_identity_image": animate_ref_identity_image, 
        }
        if animate_ref_identity_image is not None:
            units_to_run = self.units + self.face_units
        else:
            units_to_run = self.units
...
# TODO: Face Embedding, Reference: https://arxiv.org/pdf/2411.17697
class WanVideoUnit_ArcfaceEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("animate_ref_identity_image", "height", "width"),
            output_params=("ante_embeds", "ante_clip_tokens", "ante_embads_zero_padding"),
            onload_model_names=("image_encoder", "face_encoder")
        )

    def process(self, pipe: WanVideoPipeline, animate_ref_identity_image, height, width):
        if (animate_ref_identity_image is None or pipe.image_encoder is None or pipe.face_model is None or pipe.face_encoder is None):
            return {}
        pipe.load_models_to_device(self.onload_model_names)

        if isinstance(animate_ref_identity_image, (list, tuple)):
            if len(animate_ref_identity_image) == 0:
                return {}
            animate_ref_identity_image = animate_ref_identity_image[0]
        image = pipe.preprocess_image(
            animate_ref_identity_image.resize((width, height))
        ).to(pipe.device)
        clip_context = pipe.image_encoder.encode_image([image])     # torch.Size([1, 257, 1280])
        clip_context = clip_context.to(dtype=pipe.torch_dtype, device=pipe.device)
        # drop the CLS token if present
        clip_tokens = clip_context[:, 1:] if clip_context.size(1) > 1 else clip_context

        pipe.face_model.face_helper.clean_all()
        animate_ref_identity_image_bgr = cv2.cvtColor(np.array(animate_ref_identity_image), cv2.COLOR_RGB2BGR)
        face_infos = pipe.face_model.app.get(animate_ref_identity_image_bgr)
        if not face_infos:
            print(f"[WARN] No face detected. Applying zero-padding to 'ante_embeds'.")
            ante_embedding = torch.zeros((1, 512), device=pipe.device, dtype=pipe.torch_dtype)
            ante_zero_padding_signal = torch.tensor([1], device=pipe.device, dtype=torch.int32)
        else:
            ante_embedding = torch.from_numpy(face_infos[0].normed_embedding).unsqueeze(0)
            ante_embedding = ante_embedding.to(device=pipe.device, dtype=pipe.torch_dtype)
            ante_zero_padding_signal = torch.tensor([0], device=pipe.device, dtype=torch.int32)

        return {"ante_embeds": ante_embedding, "ante_clip_tokens": clip_tokens, "ante_embads_zero_padding": ante_zero_padding_signal}
...
def model_fn_wan_video(
    ...
    # Arcface Embedder
    face_encoder: FusionFaceId = None,
    animate_arcface_adapter: WanAnimateArcfaceAdapter = None,
    ante_embeds: torch.Tensor = None,
    ante_clip_tokens: torch.Tensor = None,
    **kwargs,
):
    ...
    if y is not None and dit.require_vae_embedding:
        x = torch.cat([x, y], dim=1)                                # x.shape: torch.Size([1, 36, 13, 104, 58])
    if clip_feature is not None and dit.require_clip_embedding:
        clip_embdding = dit.img_emb(clip_feature)                   # clip_feature.shape: torch.Size([1, 257, 1280]); clip_embdding.shape: torch.Size([1, 257, 5120])
        context = torch.cat([clip_embdding, context], dim=1)        # context.shape: torch.Size([1, 512, 5120]) -> torch.Size([1, 769, 5120])
    ...
    # Animate Arcface
    if ante_embeds is not None and ante_clip_tokens is not None:
        refined_face_embeds = face_encoder(ante_embeds, ante_clip_tokens, shortcut=True)

    # Patchify
    f, h, w = x.shape[2:]
    x = rearrange(x, 'b c f h w -> b (f h w) c').contiguous()   # torch.Size([Batch Size, 19604, 5120])
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            # Animate
            # if pose_latents is not None and face_pixel_values is not None and refined_face_embeds is not None:
            #     x = animate_adapter.after_transformer_block(block_id, x, motion_vec)
            #     x = animate_arcface_adapter.after_transformer_block(block_id, x, refined_face_embeds)
            # elif pose_latents is not None and face_pixel_values is not None:
            #     x = animate_adapter.after_transformer_block(block_id, x, motion_vec)

            # Animate
            if pose_latents is not None and face_pixel_values is not None:
                x = animate_adapter.after_transformer_block(block_id, x, motion_vec)
                if refined_face_embeds is not None:
                    x = animate_arcface_adapter.after_transformer_block(block_id, x, refined_face_embeds)
